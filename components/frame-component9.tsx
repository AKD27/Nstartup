import PropTypes from "prop-types";

const FrameComponent9 = ({ className = "" }) => {
  return (
    <section
      className={`self-stretch flex flex-row items-start justify-start pt-[0rem] px-[0rem] pb-[0.856rem] box-border max-w-full shrink-0 text-left text-[2rem] text-black font-aktivgrotesk-medium ${className}`}
    >
      <div className="flex-1 flex flex-row items-start justify-center pt-[1.562rem] pb-[1.187rem] pr-[1.25rem] pl-[3.75rem] box-border relative max-w-full mq1350:pl-[1.875rem] mq1350:box-border">
        <div className="h-full w-full absolute !m-[0] top-[0rem] right-[0rem] bottom-[0rem] left-[0rem] bg-background-color box-border border-[1px] border-solid border-background-color" />
        <img
          className="h-[1.875rem] w-[1.894rem] absolute !m-[0] top-[2.563rem] left-[9.925rem] z-[2]"
          alt=""
          src="/vector.svg"
        />
        <img
          className="h-[3.313rem] w-[3.35rem] absolute !m-[0] top-[2.188rem] right-[1.456rem] rounded-[50%] object-contain z-[1]"
          loading="lazy"
          alt=""
          src="/ellipse-39@2x.png"
        />
        <div className="h-[4rem] w-[4.044rem] absolute !m-[0] top-[1.75rem] left-[1.769rem] shadow-[0px_4px_4px_rgba(0,_0,_0,_0.25)] rounded-[50%] bg-primary z-[1] bg-red-600" />
        <div className="w-[74.706rem] flex flex-col items-end justify-start gap-[3rem] max-w-full mq800:gap-[1.5rem]">
          <header className="self-stretch h-[5.75rem] flex flex-col items-start justify-start gap-[2.043rem] max-w-full text-left text-[2rem] text-black font-aktivgrotesk-bold mq800:gap-[1rem]">
            <div className="flex-1 flex flex-row items-start justify-start py-[0rem] px-[0.062rem] box-border max-w-full">
              <div className="self-stretch flex flex-row items-start justify-start gap-[2.593rem] max-w-full mq800:gap-[1.313rem]">
                <div className="h-[3.5rem] w-[27.113rem] flex flex-col items-start justify-start pt-[0.375rem] px-[0rem] pb-[0rem] box-border max-w-full">
                  <button className="self-stretch flex-1 relative shadow-[0px_4px_4px_rgba(0,_0,_0,_0.25)] rounded-15xl-5 bg-background-color z-[1] border-[1px] border-solid border-text-secondary" />
                </div>
                <div className="w-[21.613rem] flex flex-col items-start justify-start pt-[1.187rem] pb-[0rem] pr-[1.25rem] pl-[0rem] box-border max-w-full">
                  <a className="[text-decoration:none] w-[13.906rem] relative text-[inherit] inline-block whitespace-nowrap z-[1]">{`Offers & Deals`}</a>
                </div>
                <div className="self-stretch w-[19.025rem] flex flex-row items-end justify-between gap-[1.25rem] text-[0.875rem] text-primary">
                  <div className="self-stretch flex flex-row items-start justify-start py-[0rem] pr-[0.687rem] pl-[0rem] gap-[0.312rem]">
                    <div className="self-stretch flex flex-col items-start justify-start pt-[0.5rem] px-[0rem] pb-[0rem]">
                      <img
                        className="w-[2.719rem] flex-1 relative max-h-full shrink-0 z-[1]"
                        alt=""
                        src="/group-21.svg"
                      />
                    </div>
                    <a className="[text-decoration:none] relative text-[inherit] inline-block min-w-[1.075rem] shrink-0 z-[1]">
                      <div className= "uu"><b>10</b></div>
                    </a>
                  </div>
                  <div className="flex flex-col items-start justify-end pt-[0rem] px-[0rem] pb-[0.393rem]">
                    <div className="flex flex-row items-start justify-start gap-[0.312rem]">
                      <div className="flex flex-col items-start justify-start pt-[0.25rem] px-[0rem] pb-[0rem]">
                        <img
                          className="w-[2.719rem] h-[2.688rem] relative shrink-0 z-[1]"
                          alt=""
                          src="/vector-1.svg"
                        />
                      </div>
                      <a className="[text-decoration:none] relative text-[inherit] inline-block min-w-[1.075rem] shrink-0 z-[1]">
                      <div className= "uu"><b>15</b></div>
                      </a>
                    </div>
                  </div>
                  <div className="flex flex-col items-start justify-end pt-[0rem] px-[0rem] pb-[0.393rem]">
                    <div className="flex flex-row items-start justify-start">
                      <div className="flex flex-col items-start justify-start pt-[0.187rem] px-[0rem] pb-[0rem]">
                        <img
                          className="w-[2.719rem] h-[2.688rem] relative shrink-0 z-[1]"
                          alt=""
                          src="/vector-2.svg"
                        />
                      </div>
                      <a className="[text-decoration:none] relative text-[inherit] inline-block min-w-[1.075rem] shrink-0 z-[1]">
                      <div className= "uu"><b>89</b></div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="self-stretch h-[0rem] relative z-[1]" />
          </header>
          <div className="w-[70.688rem] flex flex-row items-start justify-end py-[0rem] px-[3.437rem] box-border max-w-full mq1125:pl-[1.688rem] mq1125:pr-[1.688rem] mq1125:box-border">
            <div className="flex-1 flex flex-row items-start justify-between max-w-full gap-[1.25rem] mq1125:flex-wrap">
              <div className="w-[10.931rem] flex flex-col items-start justify-start gap-[0.187rem]">
                <div className="self-stretch flex flex-row items-start justify-start py-[0rem] pr-[0rem] pl-[0.187rem]">
                  <div className="flex-1 relative z-[1] mq800:text-[1.625rem] mq450:text-[1.188rem]">
                    <b>LaunchPad</b>
                  </div>
                </div>
                <div className="self-stretch h-[0.125rem] relative box-border z-[1] border-t-[2px] border-solid border-primary" />
              </div>
              <div className="w-[9.163rem] flex flex-col items-start justify-start py-[0rem] pr-[0.312rem] pl-[0rem] box-border gap-[0.375rem]">
                <div className="self-stretch flex flex-row items-start justify-start py-[0rem] pr-[0rem] pl-[0.312rem]">
                  <div className="flex-1 relative z-[1] mq800:text-[1.625rem] mq450:text-[1.188rem]">
                  <b>Learning</b>
                  </div>
                </div>
                <div className="w-[8.656rem] h-[0.125rem] relative box-border z-[1] border-t-[2px] border-solid border-primary" />
              </div>
              <div className="w-[31.919rem] flex flex-row items-start justify-start gap-[4.675rem] max-w-full mq800:flex-wrap mq800:gap-[2.313rem] mq450:gap-[1.188rem]">
                <div className="w-[11.25rem] flex flex-col items-start justify-start gap-[0.375rem]">
                  <div className="self-stretch relative z-[1] mq800:text-[1.625rem] mq450:text-[1.188rem]">
                  <b>Networking</b>
                  </div>
                  <div className="w-[10.931rem] h-[0.125rem] relative box-border z-[1] border-t-[2px] border-solid border-primary" />
                </div>
                <div className="flex-1 flex flex-row items-start justify-start gap-[1.643rem] min-w-[10.375rem]">
                  <div className="flex-1 flex flex-col items-start justify-start gap-[0.187rem]">
                    <div className="self-stretch flex flex-row items-start justify-start py-[0rem] pr-[0rem] pl-[0.125rem]">
                      <div className="flex-1 relative z-[1] mq800:text-[1.625rem] mq450:text-[1.188rem]">
                      <b>Projects</b>
                      </div>
                    </div>
                    <img
                      className="w-[8.913rem] h-[0.188rem] relative object-contain z-[1]"
                      alt=""
                    />
                  </div>
                  <div className="w-[4.931rem] flex flex-col items-start justify-start gap-[0.187rem]">
                    <div className="self-stretch relative z-[1] mq800:text-[1.625rem] mq450:text-[1.188rem]">
                    <b>Jobs</b>
                    </div>
                    <div className="w-[5.056rem] h-[0.125rem] relative box-border z-[1] border-t-[2px] border-solid border-primary" />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

FrameComponent9.propTypes = {
  className: PropTypes.string,
};

export default FrameComponent9;
